...
$(document).ready(function() {
	...
	$.ajax({
		...
		url : js_path + '/achievement/getAll/' + typeId,
...
//					blockViewData += createBlock(count, index, items.id, items.name, items.picPath, items.urlLink, items.status );
					stripViewData += createStrip(index+1, items.title, items.url);
				});
...
function createStrip(index, name, url) {
	...
	var tr = "<tr onclick='javascript: return window.location.href=&quot;" + url + "&quot;'>";
